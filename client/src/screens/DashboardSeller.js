        ...
        <div className="account-container">
          <div className="account-info-container">
            ...
            <div className="accountdetails1">
              ...
              <Link to={"/profile"} className="add-to-card-button edit-button" style={{textDecoration : "none"}}>
                Edit Account
              </Link>
            ...
            <div className="accountdetails1">
              <div className="account-details">
                <div className="account-text">
                  <h3>{infoSeller?.name}</h3>
                  <p>Country : {infoSeller?.Country}</p>
                  <p>Address : {infoSeller?.seller?.Addresse}</p>
                </div>
              </div>
            ...
            {orderLoading ? (
              ...
              <>
                <div className="Fun-Fact3">
                  ...
                  <div className="fun-icon">
                    <h3> Orders</h3>
                    <p>{orderSeller?.totalOrdersResult[0].totalOrders}</p>
                  </div>
                </div>
              ...
              {orderSeller?.orders.map((order) => (
                <tr key={order._id}>
                  ...
                  <td>
                    {order.order.isPaid ? (
                      order.order.isDelivered ? (
                        <Link to={`/detail/${order?.order._id}`} className="action-button view-details">
                          View Details ➔
                        </Link>
                      ) : (
                        <Link to={`/detail/${order?.order._id}`}  className="action-button view-details-progress">
                          View Details ➔
                        </Link>
                      )
                    ) : null}
